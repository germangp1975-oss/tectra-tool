...
def analyze_file(file, yield_limit=None):
    ...
    if not mesh.point_data:
        output.append("\nNo nodal data fields available")
        return "\n".join(output)
    ...
    if max_gradient > mean_stress:
        output.append("Significant stress gradient — possible discontinuity")

    if yield_limit is not None:
        if max_stress > yield_limit:
            output.append("Yield strength exceeded — failure risk")
        else:
